...
class plgVMPaymentMolpay extends vmPSPlugin {
    ...
    function plgVmConfirmedOrder($cart, $order) {
        ...
        $post_variables = Array(
            ...
            'merchantnumber' => $method->molpay_merchantid,
            'verifykey' => $method->molpay_verifykey,
            'country' => $country,
            ...
            "declineurl" => JROUTE::_(JURI::root() . 'index.php?option=com_virtuemart&view=pluginresponse&task=pluginUserPaymentCancel&on=' . $order['details']['BT']->order_number . '&pm=' . $order['details']['BT']->virtuemart_paymentmethod_id)
        );
    ...
    //				$orders[$virtuemart_order_id]["comments"] = '';
                    ...
                    $orders['comments'] = 'Payment successfull, Updated by Return URL';

                    //die('<pre>' . print_r($orders, true) . '</pre>');
                    $modelOrder->updateStatusForOneOrder($virtuemart_order_id, $orders, true);
                    //die('status' . var_dump($modelOrder->updateStatusForOneOrder($virtuemart_order_id, $orders, true)));
                    $cart = VirtueMartCart::getCart();
                    $cart->emptyCart();
                    $html = '<table width="50%">' . "\n";
